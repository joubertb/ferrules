...
pub fn detect_and_correct_font_corruption(
    ...
    if let Some((corrected, was_corrected)) =
        apply_real_font_corrections(text, font_name, unicode_value)
    {
...
fn get_or_analyze_font_corruption(font_name: &str) -> Option<FontCorruptionMap> {
    ...
    let pdf_data = DOCUMENT_CONTEXT.with(|ctx| {
        ctx.borrow()
            .as_ref()
            .map(|context| context.pdf_data.clone())
    });
                ...
                if let Ok(encoding_dict) = encoding_obj.as_dict() {
                    if let Ok(differences_ref) = encoding_dict.get(b"Differences") {
                        ...
                        let detected_corruptions =
                            analyze_differences_array_with_unicode_validation(
                                &doc,
                                differences_ref,
                                font_name,
                                &corruptions,
        ...
        if in_bfchar && line.starts_with('<') && line.contains("><") {
            ...
            if let Some((char_code_str, unicode_str)) = parse_cmap_mapping_line(line) {
                if let (Ok(char_code), Ok(unicode_val)) = (
                    ...
                    let corruption_status =
                        if is_character_mapping_corrupted(char_code, unicode_val) {
                            "🚨 CORRUPTED"
                        } else {
                            "✅ NORMAL"
                        };
    ...
    if let Ok(stream) = cmap_obj.as_stream() {
        ...
        match adobe_cmap_parser::get_unicode_map(&stream_data) {
            Ok(cmap) => {
                ...
                let font_name =
                    font_name_from_stream(doc, tounicode_ref).unwrap_or("Unknown".to_string());
...
fn unicode_to_char_name(unicode: u32) -> Option<String> {
    match unicode {
        ...
        // Letters (uppercase)
        0x0041 => Some("A".to_string()),
        0x0042 => Some("B".to_string()),
        0x0043 => Some("C".to_string()),
        0x0044 => Some("D".to_string()),
        0x0045 => Some("E".to_string()),
        0x0046 => Some("F".to_string()),
        0x0047 => Some("G".to_string()),
        0x0048 => Some("H".to_string()),
        0x0049 => Some("I".to_string()),
        0x004A => Some("J".to_string()),
        0x004B => Some("K".to_string()),
        0x004C => Some("L".to_string()),
        0x004D => Some("M".to_string()),
        0x004E => Some("N".to_string()),
        0x004F => Some("O".to_string()),
        0x0050 => Some("P".to_string()),
        0x0051 => Some("Q".to_string()),
        0x0052 => Some("R".to_string()),
        0x0053 => Some("S".to_string()),
        0x0054 => Some("T".to_string()),
        0x0055 => Some("U".to_string()),
        0x0056 => Some("V".to_string()),
        0x0057 => Some("W".to_string()),
        0x0058 => Some("X".to_string()),
        0x0059 => Some("Y".to_string()),
        0x005A => Some("Z".to_string()),

        // Letters (lowercase)
        0x0061 => Some("a".to_string()),
        0x0062 => Some("b".to_string()),
        0x0063 => Some("c".to_string()),
        0x0064 => Some("d".to_string()),
        0x0065 => Some("e".to_string()),
        0x0066 => Some("f".to_string()),
        0x0067 => Some("g".to_string()),
        0x0068 => Some("h".to_string()),
        0x0069 => Some("i".to_string()),
        0x006A => Some("j".to_string()),
        0x006B => Some("k".to_string()),
        0x006C => Some("l".to_string()),
        0x006D => Some("m".to_string()),
        0x006E => Some("n".to_string()),
        0x006F => Some("o".to_string()),
        0x0070 => Some("p".to_string()),
        0x0071 => Some("q".to_string()),
        0x0072 => Some("r".to_string()),
        0x0073 => Some("s".to_string()),
        0x0074 => Some("t".to_string()),
        0x0075 => Some("u".to_string()),
        0x0076 => Some("v".to_string()),
        0x0077 => Some("w".to_string()),
        0x0078 => Some("x".to_string()),
        0x0079 => Some("y".to_string()),
        0x007A => Some("z".to_string()),
...
pub fn analyze_document_fonts(pdf_data: &[u8]) -> Result<HashMap<String, FontCorruptionMap>> {
    ...
    info!(
        "Font analysis complete: {} fonts with corruption detected",
        font_maps.len()
    );
    Ok(font_maps)
}
...
mod tests {
    ...
    fn test_mathematical_font_detection() {
        assert!(is_mathematical_font("FYEQFE+NimbusRomNo9L-Regu"));
        assert!(is_mathematical_font("ABCDEF+Times-Roman"));
        assert!(is_mathematical_font("GHIJKL+ComputerModern"));
        assert!(!is_mathematical_font("ABCDEF+Helvetica"));
        assert!(!is_mathematical_font("MNOPQR+Arial"));
    }
    ...
    fn test_font_corruption_detection() {
        ...
        let (text, corrupted) = detect_and_correct_font_corruption("h", "Times-Roman", 'h' as u32);
        assert_eq!(text, "h");
        assert!(!corrupted);
    }
}
...
fn analyze_type1_builtin_font_corruption(
    doc: &Document,
...
fn analyze_encoding_for_corruption(
    doc: &Document,
    encoding_ref: &Object,
    font_name: &str,
    corruptions: &mut HashMap<u32, char>,
...
fn analyze_encoding_object(
    ...
    if let Ok(encoding_dict) = encoding_obj.as_dict() {
        ...
        let keys: Vec<String> = encoding_dict
            .iter()
...
fn analyze_differences_array_with_unicode_validation(
    ...
    if char_mappings.is_empty() {
        ...
        info!(
            "📋 No character mappings found in Differences array for font: {}",
            font_name
        );
        return corruptions;
    }
    ...
    info!(
        "🔍 Validating {} character mappings using Unicode Character Database for font: {}",
        char_mappings.len(),
        font_name
    );
    ...
    match validate_font_with_unicode_db(font_name, &char_mappings) {
        Ok(validation_result) => {
            ...
            info!(
                "✅ Unicode validation complete for {}: {}/{} mappings invalid ({:.1}% corruption)",
                font_name,
                validation_result.invalid_mappings.len(),
                validation_result.total_mappings,
                validation_result.corruption_confidence
...
fn extract_character_mappings_from_differences(
    ...
    let differences_array = match differences_ref {
        Object::Reference(reference) => match doc.get_object(*reference) {
            ...
            Err(e) => {
                warn!(
                    "❌ Failed to resolve differences reference for {}: {}",
                    font_name, e
                );
    ...
    if let Ok(array) = differences_array.as_array() {
        debug!(
            "📊 Processing Differences array for {}: {} elements",
            font_name,
            array.len()
        );
...
fn analyze_differences_array_legacy(
    doc: &Document,
    differences_ref: &Object,
    font_name: &str,
) -> HashMap<u32, char> {
    ...
    if let Object::Reference(reference) = fontdesc_ref {
        if let Ok(fontdesc_obj) = doc.get_object(*reference) {
            if let Ok(fontdesc_dict) = fontdesc_obj.as_dict() {
                ...
                let keys: Vec<String> = fontdesc_dict
                    .iter()
                    .map(|(k, _)| String::from_utf8_lossy(k).to_string())
                    .collect();
    ...
    if let Object::Reference(reference) = charprocs_ref {
        if let Ok(charprocs_obj) = doc.get_object(*reference) {
            if let Ok(charprocs_dict) = charprocs_obj.as_dict() {
                ...
                let char_names: Vec<String> = charprocs_dict
                    .iter()
                    .map(|(k, _)| String::from_utf8_lossy(k).to_string())
                    .collect();
...
fn compare_postscript_encoding_with_pdf(ps_encoding: Vec<(usize, String)>, font_name: &str) {
    ...
    for (ps_code, ps_name) in &ps_encoding {
        if let Some((_, pdf_name)) = pdf_differences
            .iter()
...
pub fn correct_assembled_text(text: &str) -> String {
    // Apply mathematical symbol corrections to assembled text (including formulas)
    super::character::fix_math_symbol_corruptions(text)
}
