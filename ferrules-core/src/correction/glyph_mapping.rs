...
impl GlyphNameResolver {
    ...
    fn needs_glyph_resolution(&self, font_name: &str, unicode_value: u32) -> bool {
        ...
        // Subset fonts (contain '+') often have corrupted mappings
        let is_subset_font = font_name.contains('+');
        
        // Focus on characters that are commonly corrupted in mathematical contexts
        let is_suspicious_unicode = matches!(unicode_value, 
            0x0068 | 0x0069 |  // h, i (often corrupted to parentheses)
            0x007B | 0x007D |  // {, } (often corrupted ligatures)  
            0x005B | 0x005D    // [, ] (bracket corruptions)
        );
        
        // Special case: CMSY fonts - both 'h' and 'i' need correction
        if font_name.contains("CMSY") {
            return matches!(unicode_value, 0x0068 | 0x0069);  // 'h' → '(', 'i' → ')'
        }
        
        // Special case: CMMI fonts - only 'h' needs correction, 'i' is already correct
        if font_name.contains("CMMI") {
            return unicode_value == 0x0068;  // Only correct 'h' → '('
        }
        
        (is_math_font || is_subset_font) && is_suspicious_unicode
    }
    
    /// Gets glyph name from system font
    /// This simulates what PDF viewers do when font glyph lookup fails
    fn get_system_font_glyph_name(&mut self, font_name: &str, unicode_value: u32) -> Option<String> {
        // Check cache first
    ...
    fn get_known_glyph_mapping(&self, font_name: &str, unicode_value: u32) -> Option<String> {
        ...
        // CMMI fonts: only 'h' is corrupted, 'i' is correct
        if font_name.contains("CMMI") {
            return match unicode_value {
                0x0068 => Some("parenleft".to_string()),    // 'h' → '(' (genuinely corrupted)
                // 0x0069 ('i') is already correct, no mapping needed
                _ => None,
            };
        }
        
        // NimbusRomNo9L subset fonts have different patterns
        if font_name.contains("NimbusRomNo9L") {
            return match unicode_value {
                0x005B => Some("fi".to_string()),           // [ → fi ligature
                0x005D => Some("fl".to_string()),           // ] → fl ligature  
                0x007B => Some("ff".to_string()),           // { → ff ligature
                0x007D => Some("ffl".to_string()),          // } → ffl ligature
                _ => None,
            };
        }
...
mod tests {
    ...
    fn test_cmsy_font_correction() {
        ...
        let result = resolver.resolve_unicode_from_glyph_name(
            "CMSY10", 
            0x0068, 
            'h'
        );
