...
impl UnicodeValidator {
    ...
    #[cfg(feature = "correction-engine")]
    fn validate_single_mapping(&mut self, char_code: u32, claimed_unicode: u32) -> Result<UnicodeValidation> {
    ...
    #[cfg(feature = "correction-engine")]
    fn analyze_character_validity(&self, char_code: u32, claimed_char: char, _claimed_unicode: u32) -> bool {
        ...
        if claimed_char.is_control() && !matches!(claimed_char, '\n' | '\r' | '\t') {
            debug!("❌ Control character detected: {:#06x} -> {:?}", char_code, claimed_char);
            return false;
        }
        
        // Check for common corruption patterns
        if self.is_likely_corruption_pattern(char_code, claimed_char) {
            debug!("❌ Corruption pattern detected: {:#06x} -> '{}'", char_code, claimed_char);
            return false;
        }
    ...
    pub fn detect_systematic_corruption(&self, validation_result: &CharMapValidationResult) -> Vec<String> {
...
mod tests {
    ...
    fn test_character_map_validation() {
        ...
        let result = validator.validate_character_map("TestFont", &char_map).unwrap();
